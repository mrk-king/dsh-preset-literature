...
import {
  appendFileSync, copyFileSync, existsSync, mkdirSync, readdirSync, readFileSync, renameSync, rmSync, writeFileSync,
} from 'node:fs'
...
export interface PaperMeta {
  ...
  createdAt: string
  updatedAt: string
  captures: Array<{ hash: string; ts: string; label?: string }>
  pdf?: { title: string; pages: number; bytes: number; extractedAt: string }
}
...
/** 拖入对话自动归档论文的默认归属文件夹。 */
export const DEFAULT_FOLDER: FolderSummary = { id: 'default', name: '默认', createdAt: '' }
...
export function paperId(title: string): string {
...
function readMeta(root: string, id: string): PaperMeta | undefined {
  const file = join(root, 'papers', id, 'meta.json')
  if (!existsSync(file)) return undefined
  try {
    return JSON.parse(readFileSync(file, 'utf8')) as PaperMeta
  } catch {
    return undefined
  }
}

function writeMeta(root: string, meta: PaperMeta): void {
...
export function ensureLibrary(root: string): LibraryIndex {
...
export function listPapers(root: string): { index: LibraryIndex; papers: PaperSummary[] } {
...
export function switchPaper(root: string, title: string): { paper: PaperSummary; created: boolean } {
  const index = ensureLibrary(root)
  const wanted = title.trim()
  const existing = index.papers.find(
...
export function listFolders(root: string): FolderSummary[] {
  return ensureLibrary(root).folders
}

/** 创建文件夹(同名已存在则直接返回);id 由名称生成。 */
export function createFolder(root: string, name: string): FolderSummary {
  const index = ensureLibrary(root)
  const wanted = name.trim()
  if (!wanted) throw new Error('folder name required')
  const existing = index.folders.find(f => f.name === wanted)
  if (existing) return existing
  const folder: FolderSummary = { id: paperId(wanted), name: wanted, createdAt: new Date().toISOString() }
...
export function removeFolder(root: string, id: string): void {
  const index = ensureLibrary(root)
...
export function renameFolder(root: string, id: string, newName: string): FolderSummary | null {
  const index = ensureLibrary(root)
...
export function setPaperFolders(root: string, id: string, folderIds: string[]): void {
  const index = ensureLibrary(root)
...
export function renamePaper(root: string, id: string, newTitle: string): PaperSummary | null {
  const index = ensureLibrary(root)
  const paper = index.papers.find(p => p.id === id)
...
export function removePaper(root: string, id: string): PaperSummary | null {
  const index = ensureLibrary(root)
...
export function purgeTrash(root: string, maxAgeMs = 30 * 24 * 3600 * 1000): number {
  const trashDir = join(root, 'trash')
...
/** 回收站内待清理的论文数量。 */
export function trashCount(root: string): number {
  const trashDir = join(root, 'trash')
  if (!existsSync(trashDir)) return 0
  return readdirSync(trashDir).filter(e => /-\d+$/.test(e)).length
}

export function currentPaper(root: string): PaperSummary | null {
  const index = ensureLibrary(root)
  if (!index.current) return null
  return index.papers.find(p => p.id === index.current) ?? null
}

export function touchPaper(root: string, id: string): void {
  const index = ensureLibrary(root)
...
export function paperDir(root: string, id: string): string {
  return join(root, 'papers', id)
}

/** Append a raw block to notes.md (caller pre-formats). */
export function appendNote(root: string, id: string, block: string): string {
...
export function appendFigure(root: string, id: string, block: string): string {
  const dir = paperDir(root, id)
  mkdirSync(dir, { recursive: true })
...
export function appendGlossary(root: string, id: string, term: string, explanation: string): string {
  const dir = paperDir(root, id)
  mkdirSync(dir, { recursive: true })
...
export function readTail(file: string, maxChars: number): string {
  if (!existsSync(file)) return ''
  const text = readFileSync(file, 'utf8')
  return text.length <= maxChars ? text : `…（前略）\n${text.slice(-maxChars)}`
}

export function readGlossary(root: string, id: string): string {
  const file = join(paperDir(root, id), 'glossary.md')
  return existsSync(file) ? readFileSync(file, 'utf8') : ''
}

export function listGlossary(root: string, id: string): Array<{ term: string; explanation: string }> {
  const raw = readGlossary(root, id)
  const out: Array<{ term: string; explanation: string }> = []
  for (const line of raw.split('\n')) {
    const m = line.match(/^\s*-\s*\*\*(.+?)\*\*\s*[—-]\s*(.+)$/)
    if (m) out.push({ term: m[1].trim(), explanation: m[2].trim() })
  }
  return out
}
...
export function rememberCapture(root: string, id: string, hash: string, label?: string): boolean {
  const meta = readMeta(root, id) ?? {
  ...
  if (meta.captures.length > 1000) meta.captures = meta.captures.slice(-1000)
  writeMeta(root, meta)
  return true
}

export function isDuplicate(root: string, id: string, hash: string): boolean {
  const meta = readMeta(root, id)
  return meta?.captures.some(c => c.hash === hash) ?? false
}

export function readPaperNotes(root: string, id: string): { notes: string; figures: string } {
  const dir = paperDir(root, id)
  return {
...
export function figureCount(root: string, id: string): number {
  const dir = join(paperDir(root, id), 'figures')
  if (!existsSync(dir)) return 0
  let n = 0
  try {
    for (const entry of readdirSafe(dir)) {
      if (/\.(png|jpe?g|webp|gif|heic|heif)$/i.test(entry)) n += 1
    }
  } catch { /* ignore */ }
  return n
}
...
export function todaysEntries(root: string, today: string): Array<{ title: string; entry: string }> {
  const index = ensureLibrary(root)
  const out: Array<{ title: string; entry: string }> = []
  for (const p of index.papers) {
...
export function findInLibrary(
  ...
  query: string,
  maxResults = 12,
): Array<{ paper: string; match: string }> {
  const index = ensureLibrary(root)
  const q = query.toLowerCase()
  const out: Array<{ paper: string; match: string }> = []
  for (const p of index.papers) {
    const file = join(paperDir(root, p.id), 'notes.md')
    if (!existsSync(file)) continue
...
// ── PDF attachment (paper.pdf + pdftotext extraction) ─────────────────────
...
function runPdfTool(bin: string, args: string[], timeoutMs = 60_000): string {
  try {
...
export function titleFromPdf(srcPath: string): string {
  const info = parsePdfInfo(srcPath)
  return (info.title?.trim() || basename(srcPath).replace(/\.pdf$/i, '')).trim() || '未命名论文'
}

function parsePdfInfo(srcPath: string): { title?: string; pages?: number; bytes?: number } {
  try {
 ...
 */
export function attachPdf(root: string, id: string, srcPath: string, callerTitle?: string): PdfMeta {
  const dir = paperDir(root, id)
  mkdirSync(dir, { recursive: true })
  ...
  const pdf: NonNullable<PaperMeta['pdf']> = {
    title,
    pages: info.pages ?? 0,
...
export function pdfMetaOf(root: string, id: string): PaperMeta['pdf'] | null {
  return readMeta(root, id)?.pdf ?? null
}

/** Absolute path of the attached PDF for a paper, or null. */
export function pdfPathOf(root: string, id: string): string | null {
  const p = join(paperDir(root, id), 'paper.pdf')
  return existsSync(p) ? p : null
}

/** Extracted PDF text (paper.txt), or '' when absent. */
export function pdfTextOf(root: string, id: string, maxChars = 40000): string {
