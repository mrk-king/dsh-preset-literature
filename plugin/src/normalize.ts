...
export function normalizePastedText(raw: string): NormalizeResult {
  ...
  for (const line of lines) {
    const t = line.trim()
    if (t.length === 0) { clean.push(''); continue }
    if (PAGE_NUMBER_RE.test(t) && t.replace(/[^\d]/g, '').length <= 4) { droppedLines += 1; continue }
    if (PAGE_X_OF_Y_RE.test(t)) { droppedLines += 1; continue }
    if (ARXIV_RE.test(t) || DOI_RE.test(t)) { droppedLines += 1; continue }
    if ((freq.get(t) ?? 0) >= 3 && t.length >= 6) { droppedLines += 1; continue }
    clean.push(line)
  }
